...
<div class="justify-content-between flex-wrap flex-md-nowrap align-items-center pl-3 pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Setting Your Profile</h1>
...
function previewImage() {
    ...
    if (image.files && image.files[0]) {
        ...
        oFReader.onload = function(oFREvent) {
            ...
            $.ajax({
                url: "{{ URL::route('homeuser.setting-user.avatar-preview') }}",
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
...
document.getElementById('update-data').addEventListener('click', function(event) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            showLoading()
            $.ajax({
                url: "{{ URL::route('homeuser.setting-user.update') }}",
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
...
document.getElementById('delete-avatar').addEventListener('click', function(event) {
    ...
    .then(result => {
        if (result.isConfirmed) {
            showLoading()
            $.ajax({
                url: "{{ URL::route('homeuser.setting-user.delete-avatar') }}",
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
